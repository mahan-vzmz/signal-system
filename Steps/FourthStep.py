...
# تنظیمات پردازش (باید مثل گام‌های قبل باشد)
low_cut = 0.5
high_cut = 45.0
notch_freq = 50.0
epoch_dur = 2.0
...
for file_path in all_files:
    ...
    try:
        ...
        # ذخیره یک کپی از داده خام برای رسم نمودار نهایی (قبل از فیلتر)
        raw_for_plot = raw.copy()

        # --- بخش ۲: پردازش‌ها (Filter -> Epoch -> AutoReject) ---
        # اعمال فیلترها
        ...
        # AutoReject (تمیزسازی اپوک‌ها)
        # تنظیمات کمی سریع‌تر برای پردازش انبوه
        ...
        # نکته مهم: در پردازش گروهی، از کانال Fp1 به عنوان نماینده چشم استفاده می‌کنیم
        # تا مولفه‌های پلک را اتوماتیک پیدا و حذف کند.
        ...
        # ==========================================
        # بخش ۴: تولید نمودارهای مقایسه‌ای (Time & Freq)
        # ==========================================
        print("   -> Creating Comparison Plots...")

        # تنظیم ابعاد تصویر کلی
        fig = plt.figure(figsize=(14, 10))

        # --- الف) رسم حوزه زمان (Time Domain) ---
        # انتخاب کانال Fp1 برای نمایش (چون تغییرات پلک را خوب نشان می‌دهد)
        target_ch = [ch for ch in epochs_final.ch_names if 'Fp1' in ch]
        picked_ch = target_ch[0] if target_ch else epochs_final.ch_names[0]

        # داده خام (۶ ثانیه اول)
        ...
        # Subplot 1: Raw
        ax1 = fig.add_subplot(3, 1, 1)  # 3 ردیف، 1 ستون، نمودار اول
        ...
        plt.close(fig)  # بستن برای جلوگیری از پر شدن حافظه
